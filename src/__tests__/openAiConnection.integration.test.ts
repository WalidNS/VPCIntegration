...
jest.mock("openai", () => {
  return {
    default: jest.fn().mockImplementation(() => ({
      models: {
        list: jest.fn().mockResolvedValue({ data: ["gpt-4", "gpt-3.5-turbo"] }),
      },
    })),
  };
});
...
describe("OpenAIConnection Integration Tests", () => {
  ...
  describe("Connection Management", () => {
    ...
    test("should connect successfully with valid API key", async () => {
      // Set a valid API key
      (connection as any).apiKey = "test-api-key";

      // Mock the testConnection method to succeed
      const originalTestConnection = connection.testConnection;
      jest.spyOn(connection, "testConnection").mockResolvedValue(true);
